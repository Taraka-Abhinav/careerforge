...
import { AlertCircle, ArrowLeft, BrainCircuit, CheckCircle2, Code2, Sparkles } from 'lucide-react';
...
export default function LearnFocus() {
  ...
  useEffect(() => {
    moduleStartedAtRef.current = Date.now();
  }, [activeModuleId]);
  ...
  const trackLearningTime = async (mod: LearningModule) => {
    if (!userId) return;
    const durationSeconds = Math.max(1, Math.round((Date.now() - moduleStartedAtRef.current) / 1000));
    await EngagementService.trackEvent(userId, 'learning_time', { moduleId: mod.id, moduleType: mod.type }, durationSeconds);
  };
  ...
  const handleCompleteModule = async (mod: LearningModule) => {
    ...
    setCompleting(true);
    try {
      const sourceType: Parameters<typeof ModuleService.completeModule>[3] =
        mod.type === 'lesson'
          ? 'lesson'
          : mod.type === 'assessment'
            ? 'assessment'
      ...
      await trackLearningTime(mod);

      if (mod.type === 'lesson') await GoalService.recordEvent(userId, 'lesson');
      if (mod.type === 'lesson') await MissionService.completeByTarget(userId, 'module');
      if (mod.type === 'project') await GoalService.recordEvent(userId, 'milestone');

      const mastered = await ModuleService.checkAndMasterSkill(userId, mod.skillId);
      if (mastered) await GoalService.recordEvent(userId, 'milestone');
  ...
  const handleQuizSubmit = async (answers: Record<string, number>) => {
    ...
    const result = await QuizService.submitAttempt(
      ...
      content.questions || [],
      answers,
      content.passThreshold || 70,
      activeModule.xpReward
    );

    setQuizResult(`Score: ${result.score}% - ${result.passed ? `+${result.xpEarned} XP` : 'Below pass threshold'}`);

    if (result.passed) {
      await ModuleService.markModuleCompleted(userId, activeModule.id);
      await trackLearningTime(activeModule);
      const mastered = await ModuleService.checkAndMasterSkill(userId, activeModule.skillId);
      if (mastered) await GoalService.recordEvent(userId, 'milestone');
    }
    ...
    <div className="min-h-screen bg-neutral-950 text-white flex flex-col">
      ...
      {view === 'problems' && userId && (
        <div className="max-w-6xl mx-auto p-6 w-full">
          ...
          <p className="text-neutral-400 text-sm mb-6">9 challenges per skill - Easy, Medium, and Hard. Real tickets, real tests.</p>
              ...
              <div className="space-y-6">
                ...
                {activeModule.status === 'completed' && (
                  <p className="text-emerald-400 font-bold flex items-center gap-2">
                    <CheckCircle2 className="w-5 h-5" /> Completed - progress saved
                  </p>
                )}
              </div>
            ) : (
